...
**调用审查门禁**：调用 z-harness 平台能力最终都落到 `zzcli` 命令，`hooks/check-zzcli-guard.js`（PreToolUse Bash，默认启用）会拦下任何含 `zzcli` 的命令并要求先向用户说明影响、待确认后重试——确保平台操作（尤其改配置/部署/上报这类有副作用的）经过人工审查。本会话临时关闭：`CASEFLOW_ZZCLI_GUARD=off`。
...
| `hooks/check-zzcli-guard.js` | caseflow↔zz-harness 协作门禁 | **调用 zz-harness 平台命令（zzcli）前的审查门禁** — Node 跨平台（**默认启用**，PreToolUse Bash）：Bash 命令含 `zzcli`（调 apollo/zzlock/zzmq/scf/mysql-check/部署/Beetle 等平台能力）即 exit 2 阻断，提示 AI 先向用户说明命令影响（查询/修改/部署/上报、哪个环境）、待用户确认后重试；`CASEFLOW_ZZCLI_GUARD=off` 本会话关闭 |
