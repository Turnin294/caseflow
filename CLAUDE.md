...
| **开始编写任何业务代码前（Java / React / Vue）** | `architecture-ddd-lite-fullstack`（默认分层规则） |
| **做技术方案设计 / 架构选型，或遇到异步解耦 / 分布式锁 / 配置 / 定时任务 / 限流 / 缓存 / RPC / HTTP / JSON / 文件存储等需求** | `zhuanzhuan-tech-stack-selection`（场景→转转自研组件选型决策 + 反模式：别用 Redis 造锁用 zzlock、别硬编码配置用 Apollo、别自己轮询用 ZZMQ、别造工具用 zzarch-common；具体 API 查知识库） |
| **开始编写或修改任何源码前（任何语言：Java / TS / JS / Python / Kotlin / Go 等）** | `coding-standards-common`（通用 7 条铁律 + 注释三档；先于具体语言 skill） |
| **完成一段业务代码、准备提测或 git commit 前** | `zhuanzhuan-code-review`（交付前自检清单：对照数据/架构/编码/SCF 通用性/重大漏洞/技术风险逐项过一遍，增量代码不符合即修正；规则引用现有 skill 不重写；团队人工流程 TAPD/审查人/40% 迁移不代办） |
...
| **设计库表 / 写 DDL / 写 SQL / 设计索引 / 用 Redis 缓存** | `zhuanzhuan-data-standards`（转转数据存储规范：MySQL 库表设计 + 索引 + SQL 优化 + DML 限制 + Redis 缓存；与 `backend-knowledge-graph-required` 正交——后者是项目级"有哪些表/怎么查"，本 skill 是通用"库表该怎么设计/SQL 该怎么写"） |
| 编写或审查 Java 代码 | `java-coding-standards` |
| **编写或审查转转任意 Java 后端代码**（SCF / WF / zzjava 工程，任意部门） | `zhuanzhuan-coding-standards`（转转研发中心通用约定：ApiResult / ErrorCode / 脱敏 / zzarch-common / 工程结构 / 日志，叠加在 java 之上、finance 之下） |
| **编写或审查金融技术部 Java 后端代码**（SCF 服务 / 聚合层 / FinanceBiz 系列基类 / 金融业务工程） | `finance-coding-standards`（叠加在 `coding-standards-common` + `java-coding-standards` + `zhuanzhuan-coding-standards` 之上，冲突时金融条款优先） |
...
| 本次会话对 caseflow 做了决策型变更（新增/删除 Skill、触发时机或核心行为变化、规则方向反转、跨 Skill 链路变化、重大团队原则沉淀） | `dev-log`（会话结束前；普通小改只写 commit body） |
| **当前 git 仓库就是 caseflow 插件源码仓库，且插件自身变更完成后工作区存在未提交变更** | `git-commit-standards`（自动 stage / commit / push） |
...
**兜底规则：** 若 Claude 即将对源码文件（`.java`、`.ts`、`.py`、`.kt` 等）执行 Edit/Write 操作，但当前会话尚未完成 `design-doc-required` 检查，必须立即停止并先触发该 skill。不存在「任务太简单可以跳过」的例外（Bug 修复、纯重构等合法例外在 skill 内部判断）。

**caseflow 收尾规则（仅限本插件源码仓库）：** 只有当当前 git 仓库就是 `caseflow` 插件源码仓库，且变更对象是 `skills/`、`hooks/`、`.claude-plugin/`、`.codex-plugin/`、`AGENTS.md`、`CLAUDE.md`、`README.md`、`docs/skill-flow*` 等插件自身文件时，才允许自动执行 `git status`、`git add -A`、`git commit`、`git push` 和插件版本号递增。业务项目即使安装了本 plugin，也绝不触发该自动收尾；业务项目提交仍按普通 `git-commit-standards` 流程等待用户确认。
...
| **③ 架构与编码门禁（实施时）** | `architecture-ddd-lite-fullstack` / `zhuanzhuan-tech-stack-selection` / `coding-standards-common` / `java-coding-standards` / `zhuanzhuan-coding-standards` / `finance-coding-standards` / `zhuanzhuan-data-standards` / `llm-agent-coding-standards` / `bugfix-coding-style` | 分层 + 每分支一 focused service / **技术栈选型决策（用哪个自研组件·别造轮子）** / 命名 + 函数原子 + 注释三档 / Java 独占条款 / **转转研发中心通用约定** / **金融技术部独占条款（部门优先）** / **数据存储规范（库表·索引·SQL·Redis）** / **LLM·Agent 集成独占条款** / bug 修复注释规范 |
| **④ 提交与日志（收尾时）** | `zhuanzhuan-code-review` / `git-commit-standards` / `daily-work-log` | 交付前自检清单（增量代码合规收口）；规范 commit；业务项目源码改动后按 bug/功能分类沉淀工作日志 |
| **⑤ 知识图谱（沉淀时）** | `backend-knowledge-graph-required` / `reverse-index-required` / `glossary-required` | 正向单服务图谱（表 / SQL / 状态机 / 原子能力 / 技术难点）/ 反向影响索引（枚举 / 字段 / 事件 / API）/ 业务术语登记 |
...
- "我要给新项目铺规范" → ⑦
- "我在改 caseflow plugin 本身" → ⑧ + ④

---

## 与 zz-harness 的协作（脑 / 手分工）

caseflow 与转转官方 `zz-harness` marketplace（`zzcommon` / `zzrd` / `zzfe` / `zzqa` / `zzapp` 等插件）**并存、职责互补**：

- **caseflow = 脑（规范与方法论）**：该怎么设计、什么是对的、编码/数据/审查规范门禁。
- **zz-harness = 手（平台动作）**：连接转转真实平台做查询/操作/修复（Apollo / zzlock / zzmq / scf / mysql-check / zzcr / fix-sonar-issues / zzjava-init 等）。

**协作原则**：caseflow 的 skill 负责"决策与规范判定"，到了需要"真查真改真验证"时，**引导调用 zz-harness 对应 skill**（各 caseflow skill 末尾「配套 zz-harness 平台能力」节列出映射）。不重复造、不互相覆盖——caseflow 不去实现平台操作，zz-harness 不重写规范知识。

| caseflow skill | 衔接的 zz-harness 能力 |
|---|---|
...
| `git-commit-standards` | beetle/commit（提交+推送+解冲突）/ beetle（CI/CD 全流程） |

> 重叠不冲突：`zhuanzhuan-data-standards` vs `mysql-check`、`zhuanzhuan-code-review` vs `zzcr` 是"规范知识 vs 平台执行"两个视角，前者在编码当下自检、后者做正式校验与上报，前后衔接。

### z-harness 工具地图（按开发场景——"我有啥工具能用"）

> 不知道某事有没有工具时，直接调 `find-zz-skills`（按场景推荐内部 skill）。以下按开发阶段分组：

**写代码 / 自测阶段**

| 想干什么 | z-harness skill |
|---|---|
| 查表真实结构（建表语句/字段清单） | `mysql` |
| 校验写的 SQL 是否合规 | `mysql-check` |
| 跑 sonar 静态扫描 + 自动修 | `fix-sonar-issues` |
| 本地代码审查出报告（传 Beetle） | `zzcr` |
| 查接口文档 / 生成前端调用代码 | `find-api-docs` / `zapi` |
| **直接调 SCF 接口自测**（不写测试代码就打接口） | `scf` |
| 查 / 改 Apollo 配置（自测开关、灰度） | `apollo` |
| 新建 zzjava 工程脚手架 | `zzjava-init` |
| curl 转 zzcli 配置 / 导接口 | `zzcli-config-generator` |

**排查 / 联调阶段**

| 想干什么 | z-harness skill |
|---|---|
| 查服务日志（按 traceId / 错误码 / 时间段） | `log-query` |
| 扫错误日志并自动修复 | `fix-error-log` / `error-fix-pipeline` |
| 查 MQ 消息 / 消费堆积 / 消息丢失 | `zzmq` |
| 查分布式锁状态 / 锁竞争死锁 | `zzlock` |
| 查定时任务执行情况 | `zzschedule` |
| 查监控指标（QPS / 耗时 / TP99） | `prometheus` |
| Pod 诊断（jstack / 线程 / OOM） | `k8s` |
| 在线 watch 方法 / trace 调用 / 反编译（Arthas） | `zzarthas` |
| 解析 traceId（时间戳 / pid） | `traceid-parser` |
| 查 SCF 服务对应的 HTTP 路由 | `zgateway` |

**CI/CD / 提交阶段**

| 想干什么 | z-harness skill |
|---|---|
| 编译 / 部署 / 门禁 / 发布全流程 | `beetle` |
| 提交 + 推送 + 解冲突 | `beetle/commit` |
| 部署失败自愈 | `fix-deploy-failure` |
| 建需求（飞书 / TAPD） | `create-story` |
| 服务瘦身 / 死代码删除 | `service-slimming` |
| 上传文件拿 URL | `media` |

> 自测一个金融接口的典型链路：`apollo`（确认配置）→ `scf`（直接打接口）→ `log-query`（看日志）→ `zzmq`（查消息投递）→ `mysql`（看数据落库）；出错则 `fix-error-log` / `zzarthas`。具体 API 用法查 `mcp__arch-kb__query_knowledge_base` 知识库。
...
| 新项目首次接入插件、`docs/design/` 完全为空 | 跳过 design-doc-required skill | **会阻断** —— 解法二选一:(1) 先创建一份占位 `docs/design/README.md`(推荐);(2) 本次会话设 `CASEFLOW_DESIGN_DOC_HOOK=off` |
...
[第 8 步] architecture-ddd-lite-fullstack  ← 强制门禁,改代码前必经(无例外)
            ↓
[第 8.5 步] zhuanzhuan-tech-stack-selection ← 涉及锁/队列/配置/定时/限流/RPC/工具选型时（用自研组件，别造轮子）
            ↓
...
[第 12 步] java-coding-standards             ← 按语言叠加
            ↓
[第 12.3 步] zhuanzhuan-coding-standards     ← 转转任意 Java 后端代码（叠加在 java 之上）
            ↓
[第 12.4 步] finance-coding-standards        ← 仅金融技术部 Java 后端代码（叠加在 zhuanzhuan 之上，冲突时金融优先）
            ↓
[第 12.6 步] zhuanzhuan-data-standards       ← 涉及库表设计 / SQL / 索引 / Redis 时（数据领域，与编码层正交）
            ↓
...
[第 13 步] markdown-writing-standards       ← 仅写 Mermaid / 结构性改 markdown 时
            ↓
[第 14.5 步] zhuanzhuan-code-review          ← 提测/commit 前交付自检（增量代码合规收口）
            ↓
...
| `java-coding-standards` vs `zhuanzhuan-coding-standards` | java 先(阿里黄山版通用),转转研发中心通用后(ApiResult/ErrorCode/脱敏/zzarch-common/工程结构/日志);叠加不替代,适用转转任意 Java 后端 |
| `zhuanzhuan-coding-standards` vs `finance-coding-standards` | 转转研发中心通用先,金融部门后;**冲突时金融优先**(优先级链路:金融 → 转转研发中心通用 → caseflow 自有 → 黄山版);注意辨伪冲突——不同框架场景(如 SCF 实现类不加 Impl vs 普通 Java 类 ServiceImpl)不是真冲突,按场景各用各的 |
| `zhuanzhuan-data-standards` vs `finance-coding-standards`(SQL 条款) | 数据存储 skill 是全公司 DBA 通用底,金融 SQL 条款(禁子查询/最多两表关联/状态变更带原状态)在其上叠加;冲突时金融优先;不重复——通用库表/索引/Redis 规则只在数据 skill,金融只写部门额外约束 |
| `zhuanzhuan-data-standards` vs `backend-knowledge-graph-required` | 正交不冲突:data-standards 是通用"库表该怎么设计/SQL 该怎么写"(规范),backend-kg 是项目级"这个项目有哪些表/字段/怎么查"(知识沉淀);写 SQL 时两者都看——先 data-standards 定规范,再 backend-kg 的 ddl-baseline 核字段 |
...
| `bugfix-coding-style` vs `coding-standards-common` | **v1.28.8 起注释红线单一来源化到 common §5.4 + §5.4.1**;bugfix-coding-style 不再独立定义注释禁令,只承担 bug 修复期独有的应用层指引(v1.17 方向反转的历史背景 / 推荐写法 Java 示例 / 摆放位置 / 适用范围 / 旧 `[DEPRECATED]` / `[ADDED]` 标记顺手清理边界 / 红色警告);两者叠加不重复 |
| `git-commit-standards` 触发后又来一次小改 commit | hook 按改动大小判定:小改放行不再过 skill;大改才强制走 |
| `daily-work-log` vs `dev-log` | daily-work-log 记业务项目改动;dev-log 仅 caseflow 自身决策;不重叠 |
...
| `architecture-ddd-lite-fullstack` | `skills/architecture-ddd-lite-fullstack/` | 编码前默认架构规则：DDD-lite 分层、Feature 模块化、单向依赖、原子能力沉淀；**与语言无关，多栈一致适用**——Java（Spring Boot/Spring Cloud）、Python（FastAPI/Django/Flask），以及 React/Vue 前端；强制代码结构清晰、易维护、低耦合、高内聚，禁止 UI / Controller / Endpoint 直接写业务逻辑或访问 DB / HTTP；**新代码落点决策**（扩展现有功能时新代码必须放到新结构暴露 public 方法，旧文件只 +1 行调用，禁止在巨型方法 / 旧骨架文件里就地追加 N 行新逻辑）；**Service 业务动作扩展铁律——每个业务分支一个 focused service，任何新方法都不进 god service**：扩展既有 service 时不允许往多分支 god service（如 `BillService` 同时承载 `repay`/`cancel`/`reject`）追加**任何** public 业务方法——新业务分支（`reverseRepay`）→ 新建该分支的 focused service；同分支变种（`partialRepay`）→ 进该分支的 focused service（若分支仍散落在 god service 则新建并把既有方法+变种**一并迁过去**）；god service 最多保留 1 行 delegate 入口且禁止写任何业务逻辑；**跨分支编排**（同一回合调用 ≥2 个 focused service 时必须落到独立 `XxxOrchestrator` / `XxxSaga`，不进任一 focused service 内部，Controller 也不直接连续调用，O1-O4 触发条件：原子事务边界 / 顺序依赖 / 失败补偿 / 复合动作）；**横切关注点豁免**（日志/审计/权限/事务/metrics/缓存/限流走 AOP/拦截器/注解统一注入，focused service 内部不重复实现；`AuditAspect` / `LoggingInterceptor` 等横切实现类不算 god service）；**服务命名 taxonomy**（同一项目内 `XxxService` / `XxxUseCase` / `XxxCommandHandler` 三种叫法只选一种贯彻；Orchestrator / Saga 与 focused service 命名解耦；禁止 `XxxApplicationService` / `XxxManager` / `XxxHelper` 等模糊命名）；**聚合边界与事务一致性**（一个 `@Transactional` 只修改一个聚合根；跨聚合走 Domain Event / Saga；用 5 问判定 Repay 是 Bill 聚合内动作还是独立聚合，模糊默认独立聚合）；**函数级业务场景分流——分支差异即拆分**（v1.26.3 新增；service 级铁律在函数粒度的下钻）：函数内按业务类型 if-else / switch 分流 ≥2 分支时,**先判定分支差异本质再决定拆分阶梯**——阶梯 1 同业务定位(共享同状态机 / 校验 / 补偿 / 团队)拆函数内私有方法 `_handleTypeA()` / `_handleTypeB()`,主方法只派发;阶梯 2 不同业务定位(独立状态机 / 独立 PRD 模块 / 独立团队)升级到 service 级,按 Service 业务动作扩展铁律拆 `AService` / `BService1`,共享逻辑沉到原子能力层;**判定锚点是「业务定位」而非「代码相似度」**——长得像但业务定位不同就要拆;1-100 期成熟项目扩展期最易违反 | DDD-lite、分层架构、Feature、原子能力、UseCase、Application、Domain、Repository、Infrastructure、结构清晰、易维护、低耦合、高内聚、前端、Spring、新代码落点、strangler pattern、旧代码堆叠禁令、Service 业务动作扩展、新业务分支必拆、同分支变种必迁、反向冲正场景、partialRepay 场景、惯性追加禁令、focused sub-service、god service 零业务方法、函数级业务场景分流、阶梯 1 私有方法、阶梯 2 升级 service、业务定位判定、if-else 堆叠禁令、1-100 期反惯性、1 行 delegate 入口、跨分支编排、Orchestrator、Saga、横切关注点豁免、AOP、拦截器、服务命名 taxonomy、CommandHandler、聚合边界、事务一致性、Domain Event、聚合 5 问 |
| `zhuanzhuan-tech-stack-selection` | `skills/zhuanzhuan-tech-stack-selection/` | **转转技术栈选型决策（编码/设计时该用哪个自研组件、别造什么轮子）**：只管「选型决策」这一层，组件 API 用法 delegate 到 `mcp__arch-kb` 知识库与运维 skill（/scf /zzmq /apollo /zzschedule /prometheus /k8s），不重复抄组件清单；场景→组件决策表（同步调用 SCF / 异步解耦 ZZMQ / 分布式锁 zzlock / 配置 Apollo / 定时 zzschedule / 限流熔断 Sentinel / 缓存 Redis·ZZ-KV / HTTP·JSON·脱敏·告警 zzarch-common / 分布式 ID ZZIdcClient / 文件 zzmedia / 对象转换 MapStruct）；反模式（Redis 造锁→zzlock / 硬编码配置→Apollo / 轮询 DB→ZZMQ / @Scheduled 写死→zzschedule / restTemplate 调内网→SCF / GsonUtil·FastJson→JsonUtil / com.bj58.idc→ZZIdcClient）；决策原则（先查 zzarch-common 再写工具 / 基础设施用平台组件 / 设计阶段标注选型 / 不确定 API 查知识库） | 技术栈、选型、自研组件、SCF、ZZMQ、RocketMQ、zzlock、分布式锁、Apollo、配置中心、zzschedule、定时任务、Sentinel、限流熔断、ZZ-KV、缓存、zzarch-common、HttpClientUtil、JsonUtil、ZZIdcClient、分布式 ID、zzmedia、文件存储、MapStruct、WxmsgUtil、DesensitizeUtil、反模式、别造轮子、Redis 造锁、硬编码配置、轮询、知识库、arch-kb |
| `zhuanzhuan-code-review` | `skills/zhuanzhuan-code-review/` | **转转金融代码交付前自检清单（增量代码合规收口，非新规则）**：提测/commit 前对照审查范围逐项过一遍，审查项规则全部引用现有 skill（数据→zhuanzhuan-data-standards、架构/基础服务交叉调用→architecture-ddd-lite-fullstack、编码/SCF 通用性→四层编码 skill、技术风险→事务发 MQ·线程安全·慢 SQL·连接打满、NPE 防护→finance §5）；6 类清单（数据库 SQL / 架构与交叉调用 / 编码与 SCF 通用性 / 重大漏洞 / 技术风险 / NPE）；增量代码不符合即修正、不符合不交付；**只承接 AI 能做的自检与合规判定**，团队人工流程（提测前一天审查、指定审查人、TAPD 录工时、掷色子抽查、周会复盘）不代办；40% 迁移线提示但由人工判断 | 代码审查、code review、交付前自检、提测、增量代码合规、审查范围、基础服务交叉调用、SCF 通用性、重大漏洞、技术风险、事务发 MQ、慢 SQL、连接打满、线程安全、NPE 防护、40% 迁移、不符合不交付、收口清单 |
| `git-commit-standards` | `skills/git-commit-standards/` | **对齐转转官方 Git Commit 规范（服务端 commit-msg + pre-receive 强制，不合法无法提交）**：type 用 `func`（非 feat）/fix/refactor/impr/perf/apm/chore/jvm/pom/conf/docs/style/test/typo/wip；scope **必填**、可多值逗号分隔、范围大用 `*`；冒号后一空格；中文 subject 不加句号；body 讲为什么；footer 放 wiki/TAPD 链接；**无 Author 行**（提交者 git 自动记录）、禁 AI 署名；caseflow 自动 push 仍受宿主命令授权策略约束；**v1.18.1 起 hook 按改动大小判定**：`hooks/check-git-commit-skill.js` 看 staged diff，≤2 文件 ∧ ≤30 行 ∧ 仅 `M` 修改时放行（让模型自行写一句 commit message），其它情况未调用本 skill 时直接 exit 2 阻断；阈值可用 `CASEFLOW_TRIVIAL_FILES` / `CASEFLOW_TRIVIAL_LINES` 调整；git push 不门禁；业务项目提交/推送/解冲突衔接 zz-harness `beetle/commit` | 提交、commit、git、func 非 feat、type、scope 必填、转转官方规范、commit-msg、pre-receive、分支、push、授权、hook、按改动大小放行、beetle/commit |
| `coding-standards-common` | `skills/coding-standards-common/` | **跨语言通用编码铁律 7 条 + 注释三档**：命名表意 / 函数原子（80 行硬阈值 + ≤4 参数 + ≤3 嵌套）/ 层次分明（单向依赖 + UI 禁直 SQL/HTTP）/ 零魔法值（DB 字段值与协议码强制枚举）/ **注释三档 + 字段可选档**（类 1-3 行 + 方法 1-2 行 + 核心块 1 行 + 字段一行可选，禁变更日志 / 禁注释代码 / TODO 必带原因负责人）+ **注释放置原则总纲**（注释只落在类/字段/方法声明上；函数体内除 §5.3 六类核心块外不写注释，靠拆函数+命名表达；每档一句话讲清，多余删）+ **§5.2.1 职责边界注释**（仅限原子能力方法 / 领域服务公开类的「职责·不负责」契约清单，每条≤1 行、≤8 条，普通 DTO/工具类不用）+ **TODO 格式 `TODO(负责人): 原因`**（负责人+原因必填、禁日期）+ **§5.0 注释语言 = 当前会话沟通语言（沟通语言一票否决）**（用户用中文沟通即写中文注释，用户用英文即写英文；**无存量文件豁免**——不沿袭原文件语言，短期内单文件中英混杂可接受、待后续重构统一；唯一能覆盖默认的合法路径是用户在本会话明确要求特定语言）/ 异常不静默 / DRY rule of 3 / **§7.6 复用项目公共能力优先**（编码前先查该项目 coding-profile 的 common-capabilities.md，禁造轮子 / 禁用原生实现替代已有公共封装）。任何源码 Edit/Write 前先满足本 skill 再走语言专属（java-coding-standards 等） | 通用编码、跨语言、命名、函数原子、80 行、嵌套、层次分明、单向依赖、零魔法值、注释三档、字段级注释、注释放置原则、函数体保持干净、声明位置注释、类注释职责边界、类头字段去重、扩展指南禁令、方法头复述代码禁令、注释密度信号、职责边界注释、原子能力契约注释、TODO 格式负责人原因、注释语言、沟通语言、中文注释、英文注释、异常、DRY、rule of 3、业务场景分流拆分、§2.5、§5.0.0 写注释 3 秒自检、§5.4 注释禁令单一来源、§5.4.1 字面反例对照表、私有方法 Javadoc 禁堆契约史、行内 WHY 硬阈值 1 行、section divider 禁令、内嵌 SQL 禁 -- 注释、SQL 口径迁外层宿主注释、旁路场景影响面归 design doc、复用公共能力优先、编码前查能力清单、common-capabilities、禁造轮子、禁原生替代公共封装、§7.6 |
...
| `zhuanzhuan-coding-standards` | `skills/zhuanzhuan-coding-standards/` | **转转研发中心通用编码约定（黄山版之上、金融层之下，适用转转任意 Java 后端）**：RPC 用 ApiResult 包装 + ApiException + 优先错误码不抛异常；ErrorCode 五位错误码枚举（-1/-2/-3 来源前缀）；Contract 实体 toString + DesensitizeUtil/@Desensitize 脱敏；zzarch-common 工具优先（禁造轮子）；JsonUtil 用 Jackson 禁 FastJson；序列化优选 hessian2 + long 转前端用 string；标准 contract/service 双模块工程结构 + maven 坐标规则（contract 禁 Spring 注解/禁枚举）；对象转换 MapStruct + lombok；日志 spring-boot-starter-log4j2 + slf4j 排冲突桥接 + 去 Location；openentry/zzmedia/权限系统 | 转转、研发中心、ApiResult、ApiException、ErrorCode、五位错误码、脱敏、DesensitizeUtil、@Desensitize、zzarch-common、禁造轮子、Jackson、禁 FastJson、hessian2、long 转 string、contract/service 双模块、maven 坐标、contract 禁注解、MapStruct、lombok、log4j2、slf4j、spring-boot-starter-log4j2、openentry、zzmedia |
| `finance-coding-standards` | `skills/finance-coding-standards/` | **金融技术部 Java 后端独占条款（部门优先，叠加在 common + java + zhuanzhuan 之上）**：优先级链路金融→转转研发中心通用→caseflow 自有；框架场景辨伪冲突（SCF 实现类不加 Impl ≠ 普通 Java ServiceImpl）；接口契约（入参出参禁 Map / 继承 FinanceBizBaseReq / ZZOpenScfBaseResult 禁 null / FinanceBizAsserts 断言 / JSR 校验）；控制流（禁 ≥2 层 if-else 嵌套，用卫语句或策略模式 / switch 必带 default）；日志分级（流程 info / 中断 warn / 异常 error）；命名（Enum/Util/Consumer/Producer 后缀 / SCF 实现类不加 Impl）；NPE 防护（拆箱 / equals / 集合元素 / RPC 返回 / 级联调用）；禁用项（旧 GsonUtil.fromJson / com.bj58.idc / ImmutablePair·Triple 用于 RPC）；数据源主从分离（读从库写主库）+ MyBatis-Plus；SQL（禁子查询 / 最多两表关联 / 无条件加 limit / 状态变更带原状态）；事务（事务中禁 RPC·HTTP / 注解仅用于方法）；数据库设计（金额用分 / 枚举英文字符串禁数字 / 是否用 yes·no / 虚拟外键用业务主键）；安全（权限校验 / 入参阈值 / 防重复触发短信） | 金融、金融技术部、SCF、聚合层、FinanceBizBaseReq、ZZOpenScfBaseResult、FinanceBizAsserts、禁 Map、if-else 嵌套、卫语句、策略模式、日志分级、Enum/Util/Consumer/Producer、SCF 实现类不加 Impl、NPE 防护、GsonUtil、ZZIdcClient、ImmutablePair、主从数据源、MyBatis-Plus、禁子查询、两表关联、limit 兜底、状态变更条件、事务禁 RPC、金额分、枚举英文字符串、yes/no、虚拟外键、部门优先、辨伪冲突 |
| `zhuanzhuan-data-standards` | `skills/zhuanzhuan-data-standards/` | **转转数据存储规范（MySQL/TiDB + Redis，数据领域独立 skill，与编码层正交）**：与 `backend-knowledge-graph-required` 互补（后者项目级"有哪些表/怎么查"，本 skill 通用"库表该怎么设计/SQL 该怎么写"）；与 `finance-coding-standards` SQL 条款互补（本 skill 全公司 DBA 通用底，金融在其上叠加）；MySQL 基础（utf8mb4 / 禁存储过程触发器外键 / 禁存大文件明文密码 / 禁业务日志实时入库）；库表设计（dbzz_xxx / t_xxx 小写下划线 / InnoDB / 字段必备注 / 金额用 DECIMAL / 禁保留字 / 单表字段 20~50）；索引（idx_/uniq_ 命名 / 主键 AUTO_INCREMENT 禁联合主键禁 UUID / 单表≤6 索引·单索引≤4 字段 / 最左前缀 / 区分度大在前 / 禁前导%·负向查询·低区分度·索引列函数运算）；SQL 优化（拆大 SQL / 禁 SELECT* / OR 改 IN≤200·IN 禁子查询 / UNION ALL / 预编译防注入 / 类型一致防隐式转换 / Limit 深分页用游标·延迟关联）；DML 限制（insert 指定字段·禁 select* / update·delete 禁无 where·须带索引字段·影响≤2 万行 / values≤2 万行 / 禁 load data）；Redis（冷热分离 QPS>5000·业务隔离·必设 TTL·大文本压缩·禁 KEYS 正则用 SCAN·MQ 用 zzmq 锁用 zzlock·谨慎全量 HGETALL/SMEMBERS·按场景选数据结构） | 数据库、MySQL、TiDB、Redis、库表设计、DDL、索引设计、SQL 优化、DML 限制、字符集、utf8mb4、dbzz_、t_、InnoDB、DECIMAL、金额、保留字、idx_、uniq_、主键、AUTO_INCREMENT、最左前缀、区分度、前导%、负向查询、SELECT*、OR 改 IN、UNION ALL、隐式转换、Limit 分页、深分页、游标、延迟关联、无 where、2 万行、缓存、冷热分离、TTL、大文本压缩、KEYS、SCAN、zzmq、zzlock、HGETALL、SMEMBERS、数据结构选型 |
...
| `dev-log` | `skills/dev-log/` | caseflow 决策型变更日志：仅记录新增/删除 Skill、触发时机或核心行为变化、规则方向反转、跨 Skill 链路变化、重大团队原则沉淀；普通小改、措辞同步、版本号递增默认由 git commit body 记录，不再写 dev-log | 开发日志、决策记录、重大规则、触发链路、规则方向反转、skill 修改、发版记录 |
...
| `bugfix-coding-style` | `skills/bugfix-coding-style/` | **v1.28.8 起注释红线单一来源化**：禁令统一收口到 `coding-standards-common` §5.4 + §5.4.1（跨语言、跨场景通用，bug 修复 / 联调 / 删冗余 / 重构 / 新功能全适用；common 触发顺序早于本 skill 不会漏读；新增反例只改 common 一处避免双地漂移）。本 skill 不再独立定义红线表，只承担 bug 修复期独有的应用层指引：**v1.17 方向反转的历史背景**（为什么禁源码内变更历史 / `[BUGFIX]` / `[DEPRECATED]` / `[ADDED]` 等标记）、**推荐写法 Java 代码示例**、**摆放位置示例**、**适用范围矩阵**、**遇到存量 `[DEPRECATED]` / `[ADDED]` 注释顺手清理的边界**、**红色警告对照表**。复杂逻辑代码块附近的 1-2 行 WHY 注释要求仍归 common §5.3 | bug修复、对齐云端、删冗余、补缺漏、bugfix style、v1.17 方向反转历史、推荐写法 Java 示例、摆放位置、适用范围矩阵、存量 [DEPRECATED]/[ADDED] 顺手清理、红色警告、规则源指向 common §5.4 |
...
| `hooks/check-commit-no-ai-signature.js` | git-commit-standards | git commit 落盘前机械扫描提交信息（-m/heredoc/-F），命中 AI 工具署名（`Co-Authored-By` 指向 AI / `noreply@anthropic.com` / 机器人 emoji / `Generated by AI`）即 exit 2 阻断 — Node 跨平台（**默认启用 block**，与改动大小/是否走 skill 无关，专防宿主默认指令擅自加签名；`CASEFLOW_AI_SIGNATURE_HOOK=warn` 仅提示、=off 关闭） |
| `hooks/check-backend-kg-readiness.js` | backend-knowledge-graph-required（兜底） | **后端业务源码 Edit 前的知识图谱阅读检查** — Node 跨平台（**v1.28 起默认启用 warn 模式**，PreToolUse Write/Edit/MultiEdit）：路径白名单 `lib/features/{module}/backend(v\d+)?/**/*.dart` 与 `lib/common/backend_infra/(daos\|services)/**/*.dart`（测试文件 `_test.dart` / `test/` 跳过）；小改豁免阈值 ≤1 文件 ∧ ≤20 行（`CASEFLOW_KG_TRIVIAL_FILES` / `CASEFLOW_KG_TRIVIAL_LINES` 可调）；扫 transcript 内 Read `**/knowledge-graph/00_index.md` 或 `**/knowledge-graph/scenarios/*.md` 字面量命中即放行；默认 `warn` 模式 exit 0 + stderr 提示，`CASEFLOW_BACKEND_KG_HOOK=block` 升级硬阻断、=off 关闭 |
| `hooks/check-comment-density.js` | coding-standards-common（兜底） | **源码注释红线机械兜底（§5.4）** — Node 跨平台（**v1.29 起默认启用，v1.35 起默认 block 硬阻断**，PreToolUse Write/Edit/MultiEdit）：仅对源码扩展名触发（`.java` / `.ts` / `.py` / `.go` 等），`.md` / `.json` / 配置文件跳过；只扫本次**新增文本**（Edit.new_string / MultiEdit.edits / Write.content），去掉字符串字面量后判定注释，不动存量注释；命中即拦：变更标记（`[BUGFIX]` / `[DEPRECATED]` / `[ADDED]`...）/ 注释里的日期 / 工单·PR·Issue 号 / 带个人或日期的 TODO / 带元信息的分节线 / 版本流水措辞等**客观红线 exit 2 硬阻断**；`long-block` 连续注释块超阈值（`CASEFLOW_COMMENT_MAX_BLOCK` 默认 6）是启发式软规则，只提示不阻断（避免误伤公开 API 长文档注释）；规则源是 `coding-standards-common` §5.4 / §5.4.1，hook 只抓客观无歧义项，prose 式实现史仍靠 skill + 评审判断；`CASEFLOW_COMMENT_HOOK=warn` 降级为仅提示、=off 关闭 |
| `hooks/scan-reverse-index.js` | reverse-index-required | 反向索引冷启动扫描器 — Node 跨平台（**默认未注册到 hooks.json,手工运行**），扫描 Java / TS 源码,识别 enum 定义 + `EnumName.VALUE` 引用 + SQL 字面量候选,产出 `states.md`,fields/events/apis 输出存根；用法 `node hooks/scan-reverse-index.js --project=. --output=./docs/knowledge-graph/reverse-index/`；输出选 `--output=user-candidates` 走用户文档目录候选池 |
