...
| **编写或审查金融技术部 Java 后端代码**（SCF 服务 / 聚合层 / FinanceBiz 系列基类 / 金融业务工程） | `finance-coding-standards`（叠加在 `coding-standards-common` + `java-coding-standards` 之上，冲突时金融条款优先） |
...
| **③ 架构与编码门禁（实施时）** | `architecture-ddd-lite-fullstack` / `coding-standards-common` / `java-coding-standards` / `finance-coding-standards` / `llm-agent-coding-standards` / `bugfix-coding-style` | 分层 + 每分支一 focused service / 命名 + 函数原子 + 注释三档 / Java 独占条款 / **金融技术部独占条款（部门优先）** / **LLM·Agent 集成独占条款** / bug 修复注释规范 |
...
[第 12 步] java-coding-standards             ← 按语言叠加
            ↓
[第 12.4 步] finance-coding-standards        ← 仅金融技术部 Java 后端代码（叠加在 java 之上，冲突时金融优先）
            ↓
...
| `java-coding-standards` vs `finance-coding-standards` | java 先(阿里黄山版通用),金融后(部门独占);**冲突时金融条款优先**(优先级链路:金融 → 转转研发中心通用 → caseflow 自有);注意辨伪冲突——不同框架场景(如 SCF 实现类不加 Impl vs 普通 Java 类 ServiceImpl)不是真冲突,按场景各用各的 |
