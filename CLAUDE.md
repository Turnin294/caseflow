...
| `hooks/check-comment-density.js` | coding-standards-common（兜底） | **源码注释红线机械兜底（§5.4）** — Node 跨平台（**v1.29 起默认启用，v1.35 起默认 block 硬阻断**，PreToolUse Write/Edit/MultiEdit）：仅对源码扩展名触发（`.dart` / `.java` / `.ts` / `.py` / `.go` 等），`.md` / `.json` / 配置文件跳过；只扫本次**新增文本**（Edit.new_string / MultiEdit.edits / Write.content），去掉字符串字面量后判定注释，不动存量注释；命中即拦：变更标记（`[BUGFIX]` / `[DEPRECATED]` / `[ADDED]`...）/ 注释里的日期 / 工单·PR·Issue 号 / 带个人或日期的 TODO / 带元信息的分节线 / 版本流水措辞等**客观红线 exit 2 硬阻断**；`long-block` 连续注释块超阈值（`CASEFLOW_COMMENT_MAX_BLOCK` 默认 6）是启发式软规则，只提示不阻断（避免误伤公开 API 长 dartdoc）；规则源是 `coding-standards-common` §5.4 / §5.4.1，hook 只抓客观无歧义项，prose 式实现史仍靠 skill + 评审判断；`CASEFLOW_COMMENT_HOOK=warn` 降级为仅提示、=off 关闭 |
