...
// CASEFLOW_COMMENT_HOOK=warn → 仅 stderr 提示，不阻断（exit 0）
// CASEFLOW_COMMENT_HOOK=off  → 完全跳过
//
// 抓的是「客观无歧义」的红线（误报极低）：
...
//
// 阈值可调：CASEFLOW_COMMENT_MAX_BLOCK（默认 6，连续注释块行数上限）
// =============================================================

const MODE = (process.env.CASEFLOW_COMMENT_HOOK || 'block').toLowerCase();
if (MODE === 'off') process.exit(0);

const MAX_BLOCK = parseInt(process.env.CASEFLOW_COMMENT_MAX_BLOCK || '6', 10);
...
process.stdin.on('end', () => {
  ...
  const lines = [
    '[caseflow] 本次新增内容的注释疑似命中 coding-standards-common §5.4 注释红线：',
    `  文件：${filePath}`,
  ];
  ...
  lines.push('  旁路：CASEFLOW_COMMENT_HOOK=warn 仅提示 / =off 关闭；CASEFLOW_COMMENT_MAX_BLOCK 调注释块行数阈值。');
