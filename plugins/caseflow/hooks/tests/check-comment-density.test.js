...
test('命中：带个人/日期的 TODO', () => {
  const { stderr } = runHook(edit('lib/a.dart', '// TODO(turnin 2026-04): 以后再优化\nfinal x = 1;\n'));
...
test('阈值可调：MAX_BLOCK=3 时 4 行注释块命中', () => {
  ...
  const { stderr } = runHook(edit('lib/a.dart', block), { CASEFLOW_COMMENT_MAX_BLOCK: '3' });
  assert.match(stderr, /long-block/);
});
