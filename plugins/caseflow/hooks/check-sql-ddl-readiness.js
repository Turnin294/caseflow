...
//   CASEFLOW_SQL_DDL_HOOK=block → 硬阻断（exit 2）
//   CASEFLOW_SQL_DDL_HOOK=off   → 完全跳过
//
...
const MODE = (process.env.CASEFLOW_SQL_DDL_HOOK || 'warn').toLowerCase();
...
process.stdin.on('end', () => {
  ...
  const msg =
    '[caseflow] 即将写/改 SQL / Mapper，但本会话尚未读过项目 DDL 基线。\n' +
    `  目标文件：${targetPath}\n` +
    ...
    '  旁路：CASEFLOW_SQL_DDL_HOOK=off 关闭 / =block 升级硬阻断。\n';
