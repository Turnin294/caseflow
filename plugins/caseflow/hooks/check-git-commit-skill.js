...
//   - 大改 → 必须先调用 caseflow:git-commit-standards skill
...
//
// 阈值可通过环境变量调整：
//   CASEFLOW_TRIVIAL_FILES（默认 2）
//   CASEFLOW_TRIVIAL_LINES（默认 30，insertions + deletions）
// =============================================================

const fs = require('fs');
const { execSync } = require('child_process');

const TRIVIAL_FILES = parseInt(process.env.CASEFLOW_TRIVIAL_FILES || '2', 10);
const TRIVIAL_LINES = parseInt(process.env.CASEFLOW_TRIVIAL_LINES || '30', 10);
...
process.stdin.on('end', () => {
  ...
  // Skill 触发时 transcript 既可能记完全限定名 "caseflow:git-commit-standards"，
  ...
  if (content.includes('git-commit-standards')
      && /"skill":"(?:caseflow:)?git-commit-standards"/.test(content)) {
    process.exit(0);
  }

  process.stderr.write(
    '[caseflow] git commit 被拦截：本次会话尚未调用 caseflow:git-commit-standards skill，' +
    `且 staged diff 不属于「小改」（阈值：≤${TRIVIAL_FILES} 文件 / ≤${TRIVIAL_LINES} 行 / 仅修改现有文件）。\n` +
    '解决方式（任选其一）：\n' +
    '  1. 若改动确实较大：先用 Skill 工具调用 caseflow:git-commit-standards 走五步清单。\n' +
    '  2. 若是误判：把改动拆小到阈值以内再 commit；或临时调高 CASEFLOW_TRIVIAL_FILES / CASEFLOW_TRIVIAL_LINES。\n'
  );
  process.exit(2);
});
