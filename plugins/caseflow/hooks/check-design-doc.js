...
//
// 项目名覆盖：
//   若 projectRoot 下存在 `.caseflow-project.json` 且含 `aiDocsProject` 字段，
...
//   <projectName> = .caseflow-project.json#aiDocsProject ?? path.basename(projectRoot)
//
// 不触发(放行,不检查):
...
//
// 旁路:
//   环境变量 CASEFLOW_DESIGN_DOC_HOOK=off 一次性禁用
//
// 退出码:
//   0 = 放行
...
if ((process.env.CASEFLOW_DESIGN_DOC_HOOK || '').toLowerCase() === 'off') {
  process.exit(0);
}
...
`[caseflow] 即将编辑源码文件，但未检测到设计文档。

目标文件：${filePath}
推断的项目根：${projectRoot}
ai-docs 子目录名：${projectName}${overridden ? '（来自 .caseflow-project.json）' : ''}
...
请在项目根创建 .caseflow-project.json：
  {"aiDocsProject": "<ai-docs 下的子目录名>"}

若本次确为极简改动且确认跳过，可设置环境变量绕过本次会话：
  PowerShell: $env:CASEFLOW_DESIGN_DOC_HOOK = 'off'
  bash/zsh:   export CASEFLOW_DESIGN_DOC_HOOK=off
...
/**
 * 解析 ai-docs 子目录名。优先读取 projectRoot/.caseflow-project.json#aiDocsProject，
 * 没有则用 path.basename(projectRoot)。
 */
function resolveProjectName(projectRoot) {
  const overrideFile = path.join(projectRoot, '.caseflow-project.json');
  try {
