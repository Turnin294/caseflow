...
| Phase 4 | 模块深度文档 + 技能卡（Vue/Spring Cloud） | 逐模块深度扫描 | 可选自动/确认 |
...
1. 读取 `pom.xml` / `build.gradle` / `package.json` — 获取技术栈和依赖
2. 探索项目目录结构 — 识别分层模式
...
1. **项目分层结构** — 根据实际项目类型选择（Spring Boot / Vue / React）
2. **依赖方向** — 层间依赖规则
3. **服务清单** — 微服务项目列出所有服务
