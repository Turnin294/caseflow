...
- **扩展既有 service:任何往多分支 god service(如 `BillService` 已有 `repay/cancel/reject`)追加 public 业务方法的动作——不论是新业务分支(`reverseRepay`)还是同分支变种(`partialRepay`)** —— 详见下面「Service 业务动作扩展铁律」专节,**god service 不加方法,每个分支去自己的 focused service**
...
5. 新方法的类注释 / Javadoc 里说明"该段未来可独立替换 / 这是 strangler pattern 第 N 段"
...
> **核心一句话**:扩展既有 service 时,**不允许往多分支 god service(如 `BillService` 同时承载 `repay`/`cancel`/`reject`)里追加任何 public 业务方法**——无论新方法是新业务分支(`reverseRepay`)还是既有分支的变种(`partialRepay`)。所有新方法必须落到**该分支自己的 focused sub-service** 里。
>
> 上一节「新代码落点决策」是通用兜底,适用所有"扩展旧代码"场景;本节是它在 service 层面的特化与下钻——**判定更硬、零退路**,因为 service 是被惯性追加最严重的容器,每一次"先在原 service 加一个"都会让下一次更难拒绝。

**核心原则**:

1. **每个业务分支聚焦到一个独立 service** —— `RepayService` 只管还款分支,`CancelService` 只管取消分支,`ReverseRepayService` 只管冲正分支。
2. **同一分支的所有方法都放在该分支自己的 service 内** —— `repay` + `partialRepay` 都属于还款分支,**两者都放在 `RepayService` 里**,作为该 service 的两个 public 方法(或参数化的同一方法);**不论原入口是不是 `BillService`,都不该往 `BillService` 里塞**。
3. **god service 只能做 1 行 delegate 入口** —— 若历史遗留的 `BillService` 同时承载多个分支动作,新方法**一律**走新建/已有的 focused sub-service;原 `BillService` 若需要保留统一入口,新方法只能 1 行 `xxxService.execute(req);`,**不允许在 `BillService` 内写任何业务逻辑**。
...
| 信号 | 说明 / 举例 |
|------|-------------|
| 业务语义上是独立动作 | `repay` / `cancel` / `reject` / `reverseRepay` 是 4 个独立分支 |
| 对应 UI / 流程图上一个独立动作节点 | 用户在前端可单独触发 / 业务流程图上独立的菱形或矩形 |
| 引入新状态机或新状态转换 | 冲正涉及账单状态机 ≠ 放款生命周期状态机 |
| 引入新下游依赖 | 新增对其它 service / 外部 HTTP / 新表的依赖 |
| 引入新事务边界 | 独立事务 / 补偿事务 / 不与原方法共事务 |
| 前置校验 / 权限规则 / 幂等键语义与既有方法不同 | "账单已出+在还款日内"≠"放款已完成" |

**变种的归属**:`repay` 与 `partialRepay` 共享同一状态机 + 同一下游 + 同一前置校验 → 属于**同一还款分支**,两者都进 `RepayService`,**不进** `BillService`。

**新方法落点决策表**:

| 新方法是什么 | 已有该分支的 focused service 吗? | 落点 |
|-------------|------------------------------|------|
| 新业务分支(如冲正) | 没有 | **新建** `ReverseRepayService`,新方法作为其 public |
| 同一分支的变种(如 partialRepay) | 已有(如 `RepayService`) | 加进该 service,作为它的第 2 个 public 方法或参数化合并 |
| 同一分支的变种(如 partialRepay) | 没有,既有方法 `repay` 还在 god service | **新建** `RepayService`,把变种 + 既有方法**一并迁过去**;god service 内对应方法降级为 1 行 delegate(或彻底删掉让调用方直接注入) |

**正确形态 — 加冲正(新分支)**:

```text
[新建] features/billing/application/ReverseRepayService.java
        - public void execute(ReverseRepayRequest req) { ... }   // 唯一 public,内部拆 _xxxStep 私有方法
        - 自己注入 BillDao / LoanDao / 状态机校验

[修改] 调用方(Controller / 上游 UseCase)直接注入 ReverseRepayService 调用;
        BillService 不新增 public 方法。

[或者]  若必须保留 BillService 作为统一入口:
        - BillService 注入 ReverseRepayService;
        - BillService 新方法只 1 行: `reverseRepayService.execute(req);`
        - 禁止把任何业务逻辑写在该 1 行方法里。
```

**正确形态 — 加 partialRepay(同分支变种,但既有 repay 还在 god service)**:

```text
[新建] features/repay/application/RepayService.java
        - public void repay(RepayRequest req) { ... }                  // 把既有 BillService.repay 迁过来
        - public void partialRepay(PartialRepayRequest req) { ... }    // 新增的变种
        - (或 refactor 成参数化的单一 repay(RepayRequest req, RepayScope scope))

[修改] 调用方直接注入 RepayService;
        BillService 中原 repay 方法降级为 1 行 delegate `repayService.repay(req)`,
...
class BillService {
    ...
    public void cancel(...) { ... }
    public void reject(...) { ... }
    public void reverseRepay(...) {  // ❌ 新业务分支 → 必须新建 ReverseRepayService
        // 任何业务逻辑都不允许
    }
}
...
class BillService {
    ...
    public void cancel(...) { ... }
    public void reject(...) { ... }
    public void partialRepay(...) {    // ❌ 即使共享还款状态机,也不放 god service
        // 应该把 repay + partialRepay 一起搬到 RepayService
    }
}
```
...
- "partialRepay 和 repay 共享状态机,所以可以放 BillService" — **错**,共享状态机说明它们属于同一还款分支,**两者都应该在 `RepayService` 里**,而不是都留在 god service
- "把既有 repay 也迁出去太大动静,先把新方法塞进来" — strangler pattern 的目的就是逐步迁出,**这次扩展就是迁出契机**
...
| 阶梯 | 触发条件 | 拆分动作 | 落点 |
|------|---------|---------|------|
| **阶梯 1(函数级)** | 函数内按业务类型 if-else / switch 分流 ≥2 个分支,**但分支共享同一业务定位**(例如 `repay` 的全额 / 部分变种,共享同一状态机和补偿) | 抽 `_handleTypeA()` / `_handleTypeB()` 私有方法,主方法只做分流派发;或重构成参数化的单一方法 | 同一 service 内 |
| **阶梯 2(service 级)** | 分支差异本质是**不同业务定位**——个租 vs 导流是 PRD / 业务概念上的不同业务实体,只是部分技术流程相似 | 升级到 service 级,按上一节「Service 业务动作扩展铁律」拆 `AService` / `BService1` | 不同 service(每个分支自己的 focused service) |
...
**1-100 期反惯性提醒**:成熟项目扩展期 AI 最容易犯的错——看到 `LoanService.handleLoan()` 已经存在,加导流贷时在里面 `else if (type == DIVERSION)` 就完事了,而不是建 `DiversionLoanService`。
...
// ❌ 反例:函数内按业务类型堆叠 ≥2 分支
class LoanService {
    public void handle(LoanRequest req) {
        if (req.type == LoanType.FINANCE_LEASE) {
            // 40 行:个租校验 + 额度占用 + 放款 + 账单生成
        } else if (req.type == LoanType.DIVERSION) {
            // 50 行:导流校验(含分期)+ 锁额度 + 分期放款 + 分期账单
        }
    }
}
```

**阶梯 1 重构**(若两种放款共享同一业务定位,只是流程参数差):
...
class LoanService {
    public void handle(LoanRequest req) {
        switch (req.type) {
            case FINANCE_LEASE -> _handleFinanceLease(req);
            case DIVERSION     -> _handleDiversion(req);
        }
    }
    private void _handleFinanceLease(LoanRequest req) { /* 40 行 */ }
    private void _handleDiversion(LoanRequest req)    { /* 50 行 */ }
}
```

**阶梯 2 重构**(若导流贷是独立业务定位——独立状态机 / 独立 PRD 模块 / 独立团队):

```java
// ✅ 阶梯 2:升级到 service 级,每个业务定位一个 focused service
class FinanceLeaseService { public void handle(FinanceLeaseRequest req) { /* 40 行 */ } }
class DiversionLoanService { public void handle(DiversionLoanRequest req) { /* 50 行 */ } }

// 调用方按放款类型注入对应 service;或保留 LoanService 作为路由器,内部只 1 行 delegate:
class LoanService {
    public void handle(LoanRequest req) {
        switch (req.type) {
            case FINANCE_LEASE -> financeLeaseService.handle(req.asFinanceLease());   // 1 行 delegate
            case DIVERSION     -> diversionLoanService.handle(req.asDiversion());     // 1 行 delegate
        }
        // 禁止在 LoanService 内写任何业务逻辑——它只能做派发
    }
}
```
...
- "导流流程 80% 和个租一样,复用最方便就是写在一起" —— 错,80% 相似是代码视角;PRD 视角是两种放款产品就要拆,共享部分沉原子能力
- "拆 service 太大动静,先放一起以后再说" —— 与 god service "先加进来,以后再拆"同源,**这次扩展就是迁出契机**
- "都是放款嘛,本来就该在 LoanService 处理" —— 错,"都是放款"是技术分类(同一聚合根),不等于"同一业务定位"
...
3. **"共享的部分能不能沉到原子能力层而不是塞进同一 public 方法?"** —— 能就升级到 service 级;共享逻辑走原子能力目录,不进任一 focused service 内部
...
> 一旦"focused service per branch"落实,新的问题就来了:用户点一个按钮"批准并放款",对应 approve + loan 两个分支需要在同一事务 / 同一回合内顺序执行——这段编排代码**不能**写在 `LoanService` 里(它就该只管放款分支),也**不能**写在 `ApproveService` 里,更**不能**留在 Controller / UI 里。它有自己的归属。
...
| # | 条件 | 举例 |
|---|------|------|
| O1 | 调用 ≥2 个 focused service 且需要**原子事务边界** | `approveAndLoan`:approve + loan 必须同事务,任一失败全部回滚 |
| O2 | 调用 ≥2 个 focused service 且有**顺序依赖**或**条件分支** | "先校验是否可放款,再决定走全额放款还是分期放款" |
| O3 | 调用 ≥2 个 focused service 且需要**失败补偿**(Saga 模式) | "放款成功但通知失败 → 不回滚放款,补一条异步重试" |
| O4 | 业务概念本身就是**一个独立可命名的复合动作** | "审核通过 + 通知客户 + 占用额度"在 UI 上是一个按钮、一个流程节点 |

**正确形态**:

```text
[新建] features/loan/application/ApproveAndLoanOrchestrator.java
        - public void execute(ApproveAndLoanRequest req) {
              approveService.approve(...);     // 调用 ApproveService 的 focused 能力
              loanService.loan(...);           // 调用 LoanService 的 focused 能力
              // orchestrator 只编排:顺序 / 事务声明 / 失败补偿 / 编排日志
              // 不写任何 approve 或 loan 的业务逻辑(那些归 focused service)
          }

[调用方] Controller / 上层 UseCase 注入 ApproveAndLoanOrchestrator 调用,
        不再直接连续调用 ApproveService + LoanService。
```
...
class LoanController {
    public void approveAndLoan(Request req) {
        approveService.approve(req);  // ❌ 编排在 Controller
        loanService.loan(req);        // ❌ 事务边界不清
    }
}

// ❌ 反例 2:把编排塞进其中一个 focused service
class LoanService {
    public void loanWithApprove(Request req) {
        approveService.approve(req);  // ❌ LoanService 不该知道 approve 分支
        // loan 逻辑...
    }
}

// ✅ 正确:独立 orchestrator
class ApproveAndLoanOrchestrator {
    @Transactional
    public void execute(ApproveAndLoanRequest req) {
        approveService.approve(req);
        loanService.loan(req);
    }
}
```
...
class RepayService {
    public void repay(req) {
        log.info("repay start, req={}", req);        // ❌ 横切走 AOP
        permissionCheck(req.userId, "REPAY");        // ❌ 走 @PreAuthorize
        auditService.record("repay", req);           // ❌ 走 @Audited + AOP
        // ... 业务逻辑
    }
}

// Java ✅ — 横切由注解 + AOP 统一注入
@Audited @PreAuthorize("hasPermission('REPAY')")
class RepayService {
    @Transactional
    public void repay(req) { /* 只写还款业务 */ }
}
```
...
class RepayService:
    def repay(self, req):
        logger.info("repay start, req=%s", req)               # ❌ 走 middleware/decorator
        if not has_permission(req.user_id, "REPAY"): raise    # ❌ 走 Depends 注入
        audit_service.record("repay", req)                    # ❌ 走 @audit decorator
        # ... 业务逻辑

# Python ✅ — 横切由 decorator + Depends 统一注入
@audit("repay")
@require_permission("REPAY")
class RepayService:
    @transactional
    def repay(self, req): pass  # 只写还款业务
```
...
| 往多分支 god service（如 `BillService` 同时承载 `repay`/`cancel`/`reject`）里追加**任何** public 业务方法——不论新方法是新业务分支（`reverseRepay`）还是同分支变种（`partialRepay`） | **新业务分支** → 新建该分支的 focused service（`ReverseRepayService`），作为其 public 方法；**同分支变种** → 若该分支已有 focused service（`RepayService`）则加进去，若分支仍散落在 god service 则新建 focused service 并把既有方法+变种**一并迁过去**。任何情况下 god service 都不新增业务方法，最多保留 1 行 delegate 入口，且禁止写任何业务逻辑。详见「Service 业务动作扩展铁律」节 |
