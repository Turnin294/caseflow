...
## 配套 zz-harness 平台能力（写完真校验）

> 本 skill 是「库表/SQL 该怎么设计」的规范知识。写完 DDL/SQL 后，**真校验合规 / 查真实表结构**用 zz-harness 的 skill——脑/手分工。

| 场景 | 用 zz-harness skill | 能做什么 |
|------|---------------------|---------|
| 写完 DDL/DML 校验是否合规 | `mysql-check`（zzrd） | 审查 SQL 是否符合转转数据库规范，真连库校验 |
| 查某张已建表的真实定义 | `mysql`（zzcommon） | 看建表语句 / DDL / 字段清单 / show create table |

> 与 [[backend-knowledge-graph-required]] 的 ddl-baseline 互补：ddl-baseline 是项目级字段速查档，`mysql` skill 是实时连库查。
